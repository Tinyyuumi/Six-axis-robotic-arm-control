...
def get_box(detections):
    ...
    for label, confidence, bbox in detections:
        ...
        if label == "apple":
            cp_to_center.append(abs(x+w/2-320))     # find the center of the apple
            apple_x_list.append(x)
            apple_y_list.append(y)
            apple_w_list.append(w)
            apple_h_list.append(h)
    ceter_apple_index = cp_to_center.index(min(cp_to_center))    #find the index of the closer apple to center. 
    apple_x = apple_x_list[ceter_apple_index]
    apple_y = apple_y_list[ceter_apple_index]
    apple_w= apple_w_list[ceter_apple_index]
    apple_h = apple_h_list[ceter_apple_index]
    return(label_list,apple_x,apple_y,apple_w,apple_h)
