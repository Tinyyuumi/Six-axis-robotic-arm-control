    ...
    def moving(self):
        while True:
            ...
            if len(cnts) >0 : #通过边缘检测来确定所识别物体的位置信息得到相对坐标
                ...
                (red_x,red_y),radius = cv2.minEnclosingCircle(cnt)
            
            mask = cv2.inRange(hsv,self.color_lower2,self.color_upper2)  #绿色
            mask = cv2.erode(mask,None,iterations=2) #显示腐蚀后的图像
            mask = cv2.GaussianBlur(mask,(3,3),0) #高斯模糊
            # res = cv2.bitwise_and(frame,frame,mask=mask) #图像合并
            cnts = cv2.findContours(mask.copy(),cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)[-2] #边缘检测
            if len(cnts) >0: #通过边缘检测来确定所识别物体的位置信息得到相对坐标
                ...
                for i in cnts:
                    if cv2.contourArea(i) > 20:
                        (gx,gy),radius = cv2.minEnclosingCircle(cnts)
                        green_x.append(gx)
                        green_y.append(gy)
                answer = []
                for i in green_x:
                    answer.append(abs(green_x[i]-red_x))
                apple_index = answer.index(min(answer))

                x = green_x[apple_index]
                y = green_y[apple_index]
